...
|-------|-------------|----------|
| GET: List all latest metric data for groups | List latest metric data for all groups.|<a href="#A1">Description</a>|
| GET: List latest metric data for a specific group | List latest metric data for a specific  group.|<a href="#A2">Description</a>|
| GET: List all latest metric data for specific group type | List latest metric data .|<a href="#1">Description</a>|
| GET: List latest metric data for Group and specific group type | List latest metric data for a specific endpoint group.|<a href="#2">Description</a>|




## [GET]: List all latest metric data {#A1}
...
```
/latest/{report}/groups?[date]&[filter]&[limit]&[strict]
```
...
URL:
```
latest/Report_B/groups?date=2015-05-01T00:00:00Z&filter=non-ok&limit=10
```
Headers:
```
x-api-key: secret_key_value
...
## [GET]: List All Metric Data for a specific  group {#A2}

This method may be used to retrieve latest metric data available in a report for a specific group. User can filer the results by status and limit the amount of results returned

### Input

```
/latest/{report}/groups/{group_name}/?[date]&[filter]&[limit]
```
...
URL:
```
latest/Report_B/groups/EL-01-AUTH?date=2015-05-01T00:00:00Z&filter=non-ok&limit=10
```
Headers:
```
x-api-key: secret_key_value
